...
noPmMessage = bk_noPmMessage = ("Hello {fname} this is an automated message\n"
                                "You are blocked for now,once im online i'll accept you"
                                "Feel free to contact me via- Evil110_bot ")
...
@userge.on_cmd("ok", about={
    ...
    'usage': "{tr}ok [username | userID]\nreply {tr}ok to a message, "
             "do {tr}ok in the private chat"}, allow_channels=False, allow_via_bot=False)
...
@userge.on_cmd("imsg", about={
    'header': "Set inline pm msg for Inline pmpermit",
    'usage': "{tr}imsg [text | reply to text msg]"}, allow_channels=False)
...
async def uninvitedPmHandler(message: Message):
    ...
    if message.from_user.id in pmCounter:
        if pmCounter[message.from_user.id] > 0:
            ...
            whomst = message.from_user.id
            whomst_peer = await userge.resolve_peer(whomst)
            await userge.send(functions.messages.DeleteHistory(peer=whomst_peer, max_id=0, revoke=True))
